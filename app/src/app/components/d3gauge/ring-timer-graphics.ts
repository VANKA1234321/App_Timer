...
export class RingTimerGraphics {
	...
	private loadDefaultConfig(): void {
		...
		const config = {
			...
			colors: {
				first: 'rgba' + d3.rgb('lightslategrey').toString().slice(3, - 1) + ', 0.2)',
				ready: 'rgba' + d3.rgb('gold').toString().slice(3, - 1) + ', 0.7)',
				countdown: 'rgba' + d3.rgb('seagreen').toString().slice(3, - 1) + ', 0.7)',
				warmup: 'orange', // orange
				warning: 'rgba' + d3.rgb('red').toString().slice(3, - 1) + ', 0.7)',
				paused: 'rgba' + d3.rgb('gold').toString().slice(3, - 1) + ', 0.7)',
				finished: 'rgba' + d3.rgb('gold').toString().slice(3, - 1) + ', 0.8)',
				stopped: 'rgba' + d3.rgb('gold').toString().slice(3, - 1) + ', 0.7)',
				text: 'rgba' + d3.rgb('black').toString().slice(3, - 1) + ', 1.0)',
			},

			warmUpFlashTime: 1000,
	...
	private drawCounterText(): void {
		...
		const singleDecValueIdx: number = this.getCounterDecValueIdx();
